...
## 지시 범위 (2026-07-30 오너 확정 · 최우선)

**지시받은 산출물까지만 한다. 실행·수정·배포는 별도 지시가 있어야 한다.**

- "만들어라" 에 **실행해서 확인하기**는 포함되지 않는다.
- 실행 중 버그를 발견해도 **고치지 않는다.** 보고하고 지시를 기다린다.
- "조사해라" 에 **고치기**는 포함되지 않는다.

이유: 확인·수정까지 붙여서 보고하고 싶은 마음에 매번 범위를 넘었고, 그때마다
오너가 지시하지 않은 변경이 저장소에 들어갔다. 말로 지켜지지 않아 기계로 막는다.

`.claude/hooks/scope-guard.sh` 가 아래를 차단한다(자체 점검: `bash .claude/hooks/selftest.sh`):

| 태그 | 막는 것 |
|---|---|
| `run-app` | 시뮬레이터/앱 실행, 검수 순회(qa-run/qa-collector), EAS 빌드 |
| `deploy` | OTA 배포, Edge Function 배포, 웹 배포, 원격 푸시 |
| `db-write` | 운영 DB 에 POST/PATCH/PUT/DELETE |

풀려면 `.claude/allowed-actions` 에 태그를 적어야 한다. **오너 지시가 있을 때만 적고,
작업이 끝나면 지운다. 태그를 추가한 사실은 보고에 반드시 적는다.**


- 단순 조사·수정·디버깅은 **직접 처리한다**(Read/Edit/Write/Bash/Grep 자유 사용). "무조건 서브에이전트에 위임" 규칙은 폐지됨(2026-07-03 오너 결정).
- 대규모 병렬 작업, 독립적 다중 작업, 광범위 코드 스윕 등 규모가 클 때만 서브에이전트(Agent)를 활용한다.
- UI/디자인 작업 시 `DESIGN_SYSTEM.md`, `AGENTS.md`를 참고한다.
