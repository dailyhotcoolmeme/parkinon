...
//
// 왜 동의(UMP)가 필요한가:
//   구글이 동의를 요구하는 지역(EEA/UK 등)에서는 동의 메시지 없이 광고를 요청하면
//   광고가 안 나가거나 AdMob 계정에 경고가 붙는다. 비개인화 광고여도 면제되지 않는다.
//   광고는 해외 로케일에서만 노출되므로(AdSlot), 동의 수집도 해외에서만 수행해
//   국내 사용자에겐 아무 변화가 없게 한다.
//
// ⚠️ 배포 국가에 유럽·영국은 없다(한국·미국·일본·캐나다·호주·뉴질랜드 6곳, 오너 결정).
//   따라서 이 코드는 "유럽 대응"이 아니라, 구글이 지역을 판단해 필요할 때만 동의를
//   띄우도록 맡겨두는 안전장치다. 실제로 일본 계정에서는 NOT_REQUIRED 로 확인됐다.
//   이 주석을 근거로 "유럽에 서비스한다"고 판단하지 말 것 — 그 오해로 한국어
//   개인정보처리방침에 GDPR 조항이 잘못 들어간 적이 있다(2026-08-01 제거).
