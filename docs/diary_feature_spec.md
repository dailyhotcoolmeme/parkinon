...
# 11. 최종 디자인 — B "따뜻한 종이 (Diarly 빈티지)" 정밀 사양 (2026-06-12, 오너 선택)

> v2 실패 원인: 본문 22px(너무 큼)·그림자 카드(앱 같음)·절제 부족. 실제 저널 앱 조사(Day One/Apple Journal/Diarly) 반영해 **제대로** 재구현. 핵심: **본문 16px·serif·넓은 행간·한 장의 페이지·절제.** 글씨 키우지 말 것.

## 11-1. 팔레트 `Journal` (B 최종, DiaryScreen 전용)
| 토큰 | HEX | 용도 |
|---|---|---|
| page | #F4EEDF | 화면 전체(부드러운 크림 종이) — 한 장의 페이지, 카드 배경 분리 안 함 |
...
| mine | #B89A52 | 내 글 표시(작은 점/마크, 큰 바 아님) |
순백·순흑·녹색 금지.

## 11-2. 타이포 (★본문 키우지 말 것)
serif = Platform.select({ios:'Georgia', android:'serif'})
| 역할 | size | weight | lineHeight | family | color |
|---|---|---|---|---|---|
| 날짜 헤더 | 22 | 600 | 30 | serif | ink |
| 요일/월(날짜 보조) | 14 | 400 | 20 | serif | inkSoft |
| **한마디 본문(읽기)** | **16** | **400** | **26** | **serif** | ink |
| 저자명 | 15 | 600 | 20 | serif | inkSoft |
| 역할 라벨 | 12 | 500 | — | sans | inkFaint |
| 하단 각주(약·약효·운동) | 13 | 500 | 20 | sans | inkFaint |
| 약효 점수칩 | 13 | 600 | — | sans | inkSoft |
| 추세보기 | 13 | 600 | — | sans | accent |
| 에디터 입력/플레이스홀더 | 16 | 400 | 26 | serif | ink/placeholder |
13px 미만 금지(각주가 하한). 본문은 16, 절대 18 이상 금지.

## 11-3. 레이아웃 — 한 장의 페이지 (카드·그림자 제거)
- 전체 배경 page. **그림자 있는 카드 금지** — 한마디는 페이지 안의 글 블록, 블록 사이는 **은은한 rule 1px 가로선 + 여백(28)** 으로만 구분.
- **날짜 헤더(상단, 우아하게)**: "6월 12일 목요일" serif 22, 가운데 또는 좌측, 아래 폭 40% rule 1px. 오늘이면 작은 accentSoft 점/칩. 좌우 날짜 이동 화살표(48 터치, inkSoft). 달력 아이콘 작게.
- **한마디(본문, 주연)**: 날짜 아래 바로. 각 작성자 블록 = [저자명 15 serif + 역할 12] 한 줄 → 본문 16 serif(행간 26) → 첨부. 내 글은 저자명 앞 작은 mine 점(•) 정도로만 표시(큰 좌측 바·테두리 금지). 블록 사이 rule 1px.
- **건강 데이터 = 페이지 하단 각주 블록**(주연 아님): 위에 rule 1px, "오늘의 기록" 같은 라벨도 아주 작게(12 inkFaint, letterSpacing) 또는 생략. 한 줄들: `💊 약 3회 · 아침6:00 점심12:00` / `😊 약효 [복용직후 4][30분 3][2시간 4]`(작은 칩 가로) / `🏃 걷기 30분`. 13px inkFaint. **우측 끝 "추세보기"**(작은 텍스트+chevron, accent). 사진/영상 행 없음.
- 사진 첨부: **액자 프레임**(surface 바탕 4~6 padding + 아주 옅은 그림자 1, radius 3) 70~76px, 가로 나열, 탭 → ImageGalleryViewer. 영상: 같은 프레임 + 중앙 play → expo-av 풀스크린.
- 음성: surface 바탕 인라인 pill(높이 44, radius 22, accent play 아이콘 + "음성 듣기" 14).
- 쓰기 진입: 큰 점선 카드 금지 → 본문 자리(또는 하단)에 조용한 한 줄 "✏️ 오늘 한마디 쓰기"(accent 14~15, 텍스트 동반). 이미 쓴 경우 "수정".

## 11-4. 작성 에디터 (펼친 일기장, 폼·녹색·큰버튼 금지)
- Modal 풀스크린 배경 pageDeep. 상단 "오늘 한마디"(serif 18) + 닫기(아이콘+"닫기").
- **글 영역 = 페이지**: 무테, surface/page 바탕, padding 18, minHeight 220, 16 serif(행간 26), 플레이스홀더 "오늘 하루는 어떠셨나요?". (선택: 아주 옅은 rule 가로 괘선 배경 — 과하면 생략)
- 첨부: 하단 조용한 한 줄 `[🎙 음성] [📷 사진] [🎬 영상]`(surface 바탕, inkSoft 14, 높이 44). 녹음 중만 accent.
- 저장: 하단 바, 버튼 **accent(세피아 테라코타)**, 미색 글자 16/700 "저장", 높이 52, radius 12. 큰 풀폭 녹색 버튼 금지.

## 11-5. 변경 핵심 (v2 대비)
1. 본문 22→**16** serif. 2. 그림자 카드 → 한 장 페이지(rule로 구분). 3. 저자 강조 좌측 큰 바 → 작은 점. 4. 건강데이터 상단 푸터 → **하단 각주**. 5. 전반적으로 더 작고·조용하고·절제. 미디어 뷰잉(ImageGalleryViewer/expo-av 풀스크린)·약효 전체점수·추세보기 위치는 유지.
