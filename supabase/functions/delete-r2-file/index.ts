...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!allowed) {
      return new Response(
        JSON.stringify({ code: ErrorCode.FORBIDDEN_DELETE, error: 'not permitted to delete this object' }),
    ...
    return new Response(
      JSON.stringify({ code: ErrorCode.DELETE_FAILED, error: 'delete failed' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    );
  }
});
